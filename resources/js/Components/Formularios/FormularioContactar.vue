<template>
  <form @submit.prevent="submitContactar">
    ...
    <div class="mb-3">
      <input type="text" class="form-control" v-model="formData.nombre" placeholder="Nombre" name="nombre" required />
    </div>
    <div class="mb-3">
      <input type="text" class="form-control" v-model="formData.apellidos" placeholder="Apellidos" name="apellidos" />
    </div>
    <div class="mb-3">
      <input type="email" class="form-control" v-model="formData.email" required placeholder="Correo Electrónico"
        name="email" />
    </div>
    <div class="mb-3">
      <input type="tel" class="form-control" v-model="formData.telefono" placeholder="Teléfono" name="telefono" />
    </div>
    <div class="mb-3">
      <textarea class="form-control" v-model="formData.mensaje" required placeholder="Escribe tu mensaje" rows="3"
        name="mensaje"></textarea>
    </div>
    <div class="form-check mb-3 contain-privacidad">
      <input type="checkbox" class="form-check-input" id="privacidad" v-model="formData.privacidad" required
        name="privacidad" />
      <label class="form-check-label privacidad" for="privacidad">
        He leído y acepto la <a href="/politicas/privacidad" target="_blank">Política de Privacidad</a> y del <a href="/tratamiento-formularios" target="_blank">Tratamiento de Formularios</a>
      </label>
    </div>
    <button type="submit" class="btn-public w-100" :disabled="!formData.privacidad">
      Enviar
    </button>
  </form>
</template>
